...
class TestExecutionFlowIntegration:
    ...
    def test_execution_button_trigger_flow(self, mock_session_state):
        ...
        with patch.object(st, "session_state", mock_session_state):
            with patch("streamlit.switch_page") as mock_switch_page:
                ...
                assert st.session_state.app_state.user_inputs["script_content"] == script_content
                assert st.session_state.selected_format == "PDF"
                assert st.session_state.should_start_generation is True
                
                # Verify navigation was called
                mock_switch_page.assert_called_once_with("frontend/components/pages/result_page.py")

    def test_llm_generation_process_integration(self, mock_session_state, mock_template):
        ...
        with patch.object(st, "session_state", mock_session_state):
            ...
            # Test with real SlideGenChain (if available)
                ...
                with patch("src.backend.services.slides_loader.SlidesLoader.create_slide_functions_summary") as mock_catalog:
                    with patch("src.backend.services.slides_loader.SlidesLoader.load_template_functions") as mock_functions:
                        with patch("src.backend.services.slides_loader.SlidesLoader.get_function_by_name") as mock_get_function:
                            # Setup mocks
                            mock_catalog.return_value = "Function catalog"
                            mock_functions.return_value = {
                                "title_slide": {"name": "title_slide", "params": ["title"]}
                            }
                            mock_get_function.return_value = lambda **kwargs: "# Test Slide\nContent"
    ...
    def test_error_handling_during_generation(self, mock_session_state, mock_template):
        ...
        with patch.object(st, "session_state", mock_session_state):
            ...
            error_generator.invoke_slide_gen_chain.side_effect = Exception("LLM connection error")
...
"""
            ...
            with patch("src.backend.services.slides_loader.SlidesLoader.load_template_functions") as mock_functions:
                with patch("src.backend.services.slides_loader.SlidesLoader.get_function_by_name") as mock_get_function:
                    ...
                    try:
                        ...
                        pytest.skip(f"Other exception encountered (this is acceptable): {e}")
    ...
    def test_session_state_cleanup_after_generation(self, mock_session_state):
        ...
        with patch.object(st, "session_state", mock_session_state):
            ...
            assert st.session_state.app_state.generated_markdown == "# Generated Content"
        ...
        with patch.object(st, "session_state", mock_session_state):
            ...
            for format_name in formats:
                ...
                assert format_options[format_name]["format"] in [
                    OutputFormat.PDF, OutputFormat.HTML, OutputFormat.PPTX
                ]

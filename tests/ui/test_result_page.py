...
class TestResultPageLogic:
    ...
    def test_redirect_logic_when_no_app_state(self):
        ...
        with patch('streamlit.switch_page') as mock_switch_page:
            ...
            with patch.object(st, 'session_state', MagicMock()) as mock_session:
                ...
                if (not hasattr(st.session_state, "app_state") or
                    st.session_state.app_state.selected_template is None or
                    st.session_state.app_state.generated_markdown is None or
                    "selected_format" not in st.session_state):
                    st.switch_page("components/pages/gallery_page.py")
                
                # Verify redirect was called
                mock_switch_page.assert_called_once_with("components/pages/gallery_page.py")
    ...
    def test_navigation_button_logic(self):
        ...
        with patch('streamlit.switch_page') as mock_switch_page:
            ...
            # Verify navigation
            mock_switch_page.assert_called_with("components/pages/implementation_page.py")
